...
internal class BoardGetController(
    ...
    // --- 게시글 목록 조회 UseCase (Offset - Read 서버용 Fallback?) ---
    // 참고: 이 UseCase는 Read 서비스 로직 분석 결과 사용되지 않을 수 있음
    private val fetchBoardListForReadServerByOffset: FetchBoardListForReadServerByOffset, // 필요 없다면 제거
    ...
    fun fetchBoardsBeyondCache(
        ...
        val request = FetchBoardListForReadServerByOffset.Request(
            page = page, size = size, categoryId = categoryId, author = author, keyword = keyword
        )
    ...
    fun fetchRankingBoardIds(
        ...
        // Create UseCase Request with Enums
    ...
    fun fetchRankingBoardIdsAfter(
        ...
        // lastScore 파라미터 제거
        // @Parameter(description = "마지막 게시글 점수", example = "42.5")
        // @RequestParam(required = false) lastScore: Double?,
