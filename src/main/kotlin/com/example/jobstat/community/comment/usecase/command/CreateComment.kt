...
import jakarta.validation.constraints.NotNull // ExecuteRequest의 boardId에 필요
...
internal class CreateComment(
    ...
    validator: Validator,
) : ValidUseCase<CreateComment.ExecuteRequest, CreateComment.Response>(validator) { // Generic 타입을 ExecuteRequest로 변경

    @Transactional
    override fun invoke(request: ExecuteRequest): Response { // 파라미터 타입을 ExecuteRequest로 변경
    ...
    override fun execute(request: ExecuteRequest): Response {
        ...
        communityCommandEventPublisher.publishCommentCreated(
            ...
            boardId = request.boardId, // request에서 boardId 사용
            userId = userId
        )
    ...
    data class Response(
        ...
        companion object {
            fun from(comment: Comment): Response = with(comment) {
                Response(
                    id = id.toString(),
                    boardId = board.id.toString(), // comment 엔티티는 board 객체를 가지고 있음
                    author = author,
                    content = content,
