...
internal class CounterBatchService(
     ...
     * Redis 키는 이 메소드 호출 전에 이미 삭제됨 (by CounterService using CounterRepository).
    ...
    fun processSingleBoardCounter(boardId: Long, viewCount: Int, likeCount: Int): Boolean {
        return try {
            ...
            if (viewCount == 0 && likeCount == 0) {
                ...
                board.incrementViewCount(viewCount) // Board 엔티티에 해당 메소드 구현 필요
                board.incrementLikeCount(likeCount) // Board 엔티티에 해당 메소드 구현 필요
                if (viewCount > 0) { // 또는 if (viewChanged)
                    communityCommandEventPublisher.publishBoardViewed(
                        boardId = boardId,
                        createdAt = board.createdAt ?: LocalDateTime.now(), // 예시: createdAt 필드 사용
                        viewCount = board.viewCount // 예시: viewCount 필드 사용
                    )
                }
