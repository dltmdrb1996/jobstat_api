...
class CommunityEventHandlerVerLua(
    ...
    override fun handleBoardCreated(payload: BoardCreatedEventPayload) {
        ...
        } catch (e: Exception) { handleException(e, "게시글 생성", "boardId=${payload.boardId}") }
    }

    override fun handleBoardUpdated(payload: BoardUpdatedEventPayload) {
        try {
            val currentBoard = boardDetailRepository.findBoardDetail(payload.boardId) ?: run {
                log.warn("게시글 수정 처리 실패: 원본 데이터 없음. boardId=${payload.boardId}")
                return
            }
            val updatedBoard = currentBoard.copy(
                title = payload.title, content = payload.content, eventTs = payload.eventTs
            )
            val updatedBoardJson = dataSerializer.serialize(updatedBoard) ?: run {
                log.error("게시글 수정 데이터 직렬화 실패: boardId={}", payload.boardId); return
            }
        ...
        } catch (e: Exception) { handleException(e, "게시글 수정", "boardId=${payload.boardId}") }
    }
    ...
    override fun handleBoardDeleted(payload: BoardDeletedEventPayload) {
        ...
        } catch (e: Exception) { handleException(e, "게시글 삭제", "boardId=${payload.boardId}") }
    }

    override fun handleBoardLiked(payload: BoardLikedEventPayload) {
        try {
            val currentBoard = boardDetailRepository.findBoardDetail(payload.boardId) ?: run {
                log.warn("게시글 좋아요 처리 실패: 원본 데이터 없음. boardId=${payload.boardId}"); return
            }
            val updatedBoard = currentBoard.copy(likeCount = payload.likeCount, eventTs = payload.eventTs)
            val updatedBoardJson = dataSerializer.serialize(updatedBoard) ?: run {
                log.error("게시글 좋아요 데이터 직렬화 실패: boardId={}", payload.boardId); return
            }
        ...
        } catch (e: Exception) { handleException(e, "게시글 좋아요 업데이트", "boardId=${payload.boardId}") }
    }

    override fun handleBoardViewed(payload: BoardViewedEventPayload) {
        try {
            val currentBoard = boardDetailRepository.findBoardDetail(payload.boardId) ?: run {
                log.warn("게시글 조회수 처리 실패: 원본 데이터 없음. boardId=${payload.boardId}"); return
            }
            val updatedBoard = currentBoard.copy(viewCount = payload.viewCount, eventTs = payload.eventTs)
            val updatedBoardJson = dataSerializer.serialize(updatedBoard) ?: run {
                log.error("게시글 조회수 데이터 직렬화 실패: boardId={}", payload.boardId); return
            }
        ...
        } catch (e: Exception) { handleException(e, "게시글 조회수 업데이트", "boardId=${payload.boardId}") }
    }
    ...
    override fun handleBoardRankingUpdated(payload: BoardRankingUpdatedEventPayload) {
        ...
        } catch (e: Exception) { handleException(e, "게시글 랭킹 업데이트", "metric=${payload.metric}, period=${payload.period}") }
    }
    ...
    override fun handleCommentCreated(payload: CommentCreatedEventPayload) {
        try {
            ...
            val updatedBoardJson: String? = if (currentBoard != null) {
                dataSerializer.serialize(currentBoard.copy(commentCount = currentBoard.commentCount + 1))
            } else {
                log.warn("댓글 생성 시 게시글 정보 업데이트 실패: 원본 게시글 없음. boardId=${payload.boardId}")
                null
            }
            val commentJson = dataSerializer.serialize(payload.toReadModel()) ?: run {
                log.error("댓글 생성 데이터 직렬화 실패: commentId={}", payload.commentId); return
            }
        ...
        } catch (e: Exception) { handleException(e, "댓글 생성", "commentId=${payload.commentId}") }
    }

    override fun handleCommentUpdated(payload: CommentUpdatedEventPayload) {
        try {
            val currentComment = commentDetailRepository.findCommentDetail(payload.commentId) ?: run {
                log.warn("댓글 수정 처리 실패: 원본 댓글 없음. commentId=${payload.commentId}"); return
            }
            val updatedComment = currentComment.copy(
                content = payload.content, updatedAt = payload.updatedAt, eventTs = payload.eventTs
            )
            val updatedCommentJson = dataSerializer.serialize(updatedComment) ?: run {
                log.error("댓글 수정 데이터 직렬화 실패: commentId={}", payload.commentId); return
            }
        ...
        } catch (e: Exception) { handleException(e, "댓글 수정", "commentId=${payload.commentId}") }
    }
    ...
    override fun handleCommentDeleted(payload: CommentDeletedEventPayload) {
        try {
            val currentBoard = boardDetailRepository.findBoardDetail(payload.boardId)
            val updatedBoardJson: String? = if (currentBoard != null) {
                dataSerializer.serialize(currentBoard.copy(commentCount = (currentBoard.commentCount - 1).coerceAtLeast(0)))
            } else {
        ...
        } catch (e: Exception) { handleException(e, "댓글 삭제", "commentId=${payload.commentId}") }
    }

    private fun logOutcome(success: Boolean, operation: String, context: String) {
        if (success) {
    ...
    private fun handleException(e: Exception, operation: String, context: String) {
