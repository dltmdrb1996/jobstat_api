...
interface CommunityReadService {
    ...
    fun getLatestBoardsByCursor(lastBoardId: Long?, limit: Long): List<BoardReadModel>

    // --- 카테고리별 게시글 조회 ---
    fun getCategoryBoardsByOffset(categoryId: Long, pageable: Pageable): Page<BoardReadModel>
    fun getCategoryBoardsByCursor(categoryId: Long, lastBoardId: Long?, limit: Long): List<BoardReadModel>
     ...
     */
    fun getRankedBoardsByOffset(metric: BoardRankingMetric, period: BoardRankingPeriod, pageable: Pageable): Page<BoardReadModel>
     ...
     * @param lastScore 마지막으로 조회된 게시글의 점수 (DB Fallback 시 정확도 향상을 위해 사용될 수 있음 - TODO 확인)
     * @param limit 조회할 개수
     * @return 게시글 목록
     */
    fun getRankedBoardsByCursor(metric: BoardRankingMetric, period: BoardRankingPeriod, lastBoardId: Long?, lastScore: Double?, limit: Long): List<BoardReadModel>


    fun getCommentsByBoardIdByOffset(boardId: Long, pageable: Pageable): Page<CommentReadModel>
    fun getCommentsByBoardIdByCursor(boardId: Long, lastCommentId: Long?, limit: Long): List<CommentReadModel>
