...
object RedisOperationUtils {
    ...
    inline fun <T> executeWithRetry(
        ...
        while (retryCount < MAX_RETRY_COUNT) {
            try {
                ...
                logger.warn(
                    "Redis 작업 실패 (재시도 {}/{}): {} - {}. 원인: {}", 
                    retryCount, MAX_RETRY_COUNT, operationName, detailInfo, e.message
                )
        ...
        logger.error(
            ...
            operationName, detailInfo, lastException
        )
    ...
    fun calculateSecondsUntilMidnight(): Long {
        ...
        return java.time.Duration.between(now, tomorrow).seconds
    }
}