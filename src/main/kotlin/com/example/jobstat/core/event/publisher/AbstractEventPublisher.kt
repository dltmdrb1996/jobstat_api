...
abstract class AbstractEventPublisher(
     ...
     */
    override fun publish(type: EventType, payload: EventPayload) {
        log.info("[{}] 이벤트 발행 시도: type=${type}, payloadType=${payload::class.simpleName}",
        ...
        try {
            ...
            log.error("[{}] 이벤트 발행 중 오류 발생: type=${type}, error=${e.message}", 
    ...
    private fun validateEventType(type: EventType) {
        ...
        require(type in supportedTypes) {
            log.warn("[{}] 지원하지 않는 이벤트 타입: type={}, supported={}", 
                this::class.simpleName, type, supportedTypes)
