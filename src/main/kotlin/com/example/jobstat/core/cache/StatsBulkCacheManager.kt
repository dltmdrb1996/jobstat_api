...
import com.example.jobstat.core.cache.CacheConfig.Companion.STATS_DOCUMENT_CACHE
...
//@EnableScheduling
@Component
class StatsBulkCacheManager {
    private val log = LoggerFactory.getLogger(this::class.java)

    // 별도의 타입 안전한 캐시 인스턴스 생성 (필요한 타입 캐스팅 최소화)
    private val statsDocumentCache: Cache<String, BaseStatsDocument> = Caffeine
        .newBuilder()
        .maximumSize(CacheConfig.STATS_CACHE_SIZE)
        .expireAfterWrite(CacheConfig.EXPIRE_AFTER_ACCESS)
        .recordStats()
        .build()
     ...
     */
    fun put(key: String, document: BaseStatsDocument) {
    ...
    fun <T : BaseStatsDocument> get(key: String): T? {
        return statsDocumentCache.getIfPresent(key) as T?
    }
     ...
     * 캐시 키 생성 헬퍼 메소드
     */
    fun createCacheKey(baseDate: BaseDate, statsType: StatsType, entityId: Long): String {
