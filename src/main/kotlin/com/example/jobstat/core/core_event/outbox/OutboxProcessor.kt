...
import com.example.jobstat.core.core_event.dlt.CustomDltHeaders
...
// @Component // 제거
// @Transactional // 클래스 레벨 트랜잭션은 신중히. 각 public 메소드에 필요시 적용
open class OutboxProcessor(
    ...
    @Transactional(propagation = Propagation.REQUIRES_NEW) // 각 아이템을 새 트랜잭션에서 처리
    ...
    private fun retryEventExecuteInternal(outbox: Outbox) {
        try {
            log.debug("Kafka 메시지 발행 시도: id={}, topic={}", outbox.id, outbox.eventType.topic)

            val sendResult =
                outboxKafkaTemplate
                    .send(outbox.eventType.topic, outbox.id.toString(), outbox.event)
            ...
            log.info( // 성공 시 INFO 레벨
                ...
                outbox.id, outbox.eventType.topic, recordMetadata.partition(), recordMetadata.offset()
            )

            outboxRepository.deleteById(outbox.id) // 현재 트랜잭션 내에서 삭제
            log.debug("Outbox 레코드 삭제 완료: id={}", outbox.id)

        } catch (e: Exception) { // Kafka 발행 실패 (Timeout, Interrupted, ExecutionException 등)
            ...
            log.error(
                ...
                outbox.id, outbox.eventType, outbox.retryCount, errorMessage, e
            )
        ...
        if (nextRetryCount >= maxRetryCount) {
            ...
            try {
                ...
                log.debug(
                    ...
                    outbox.id, outbox.eventType, outbox.retryCount
                )
            } catch (e: Exception) {
    ...
    private fun attemptDltSendAndUpdateStatus(
        ...
        try {
            ...
            log.info( // DLT 전송 성공은 INFO 레벨
                ...
                outbox.id, dltTopic, sendResult.recordMetadata?.offset()
            )
            ...
            log.error(
                ...
                outbox.id, outbox.retryCount, dltTopic, e.message, e
            )
