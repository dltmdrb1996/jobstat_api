...
import com.wildrew.jobstat.statistics_read.rankings.model.rankingtype.RemoteWorkTypeRankingType
import com.wildrew.jobstat.statistics_read.rankings.service.RankingAnalysisService
import com.wildrew.jobstat.statistics_read.stats.document.RemoteWorkTypeStatsDocument
...
@Service
class GetRemoteWorkTypeRankingWithStats(
    private val rankingAnalysisService: RankingAnalysisService,
    private val validator: Validator,
) : UseCase<GetRemoteWorkTypeRankingWithStats.Request, GetRemoteWorkTypeRankingWithStats.Response> {
    companion object {
        val REMOTE_WORK_TYPE_RANKING_TYPES =
            setOf(
    ...
    fun execute(request: Request): Response =
        with(request) {
            rankingAnalysisService
                .findStatsWithRanking<RemoteWorkTypeStatsDocument>(
    ...
    data class Request(
        @field:NotNull val rankingType: RemoteWorkTypeRankingType,
        @field:NotNull val baseDate: BaseDate,
        val cursor: Int? = null,
        @field:Max(100) val limit: Int = 20,
    )

    @Schema(name = "GetRemoteWorkTypeRankingWithStatsResponse", description = "원격 근무 형태 순위 조회 응답")
    data class Response(
        @Schema(description = "순위 타입", example = "REMOTE_WORK_TYPE_POSTING_COUNT")
        ...
        val items: List<RankingWithStats<RemoteWorkTypeStatsDocument>>,
    )
}
